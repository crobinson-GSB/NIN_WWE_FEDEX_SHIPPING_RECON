...
st.caption("Drop all vendor files here — NIN, WWE, and FedEx are detected automatically. You can upload multiple files per vendor.")
vendor_files = st.file_uploader(
    "Vendor invoice files (NIN, WWE/UPS, FedEx)",
    type=["xls", "xlsx", "csv"],
    key="vendors",
...
def build_excel(merged, has_nin, has_wwe, has_fedex, vendors_label, period_label, dup_df, vision_raw):
...
if st.button("Run Reconciliation", disabled=not ready):
    with st.spinner("Matching invoices and calculating discrepancies..."):
        try:
            ...
            if not (has_nin or has_wwe or has_fedex):
                raise ReconError(
                    code="RUN-001",
                    title="No recognizable vendor files uploaded",
                    detail="None of the uploaded files could be identified as a NIN, WWE, or FedEx invoice.",
            ...
            excel_buf = build_excel(merged, has_nin, has_wwe, has_fedex,
